class CubeObject:
    ...
    def create_start_line(self,length, size_z):
        lines = []
    ...
    def build_line(self,size_x, index, sym, length):
        line = ''
    ...
    def build_3d(self,size_x, size_y, size_z):
        ...
        lines = self.create_start_line(length_y, size_z)
        index = 0
        i = 1
        new_lines = []
        while index <= length_y:
            line = ''
            if size_z > index:
                line = self.build_line(size_x, index, '/', length_y)
            elif size_z == index:
                line = self.build_line(size_x, 0, '|', length_y)
            elif size_z < index:
                line = self.build_line(size_x, index, '|', length_y)
